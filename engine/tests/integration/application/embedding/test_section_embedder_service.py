...
def test_embeds_all_unembedded_sections(db_session, embedder):
    ...
    repo.bulk_save([
        Section(text="s1", clear_language="s1", target_id=target.id),
        Section(text="s2", clear_language="s2", target_id=target.id),
    ])
...
def test_target_id_filter_embeds_only_that_targets_sections(db_session, embedder):
    ...
    repo.bulk_save([
        ...
        Section(text="b", clear_language="b", target_id=t1.id),
        Section(text="c", clear_language="c", target_id=t2.id),
    ])
